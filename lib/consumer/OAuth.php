 ...
 */
class ConsumerOAuthException extends Exception { /*{{{*/
// pass
} /*}}}*/
	...
	public function to_header() { /*{{{*/
		...
		foreach ( $this->parameters as $k => $v ) {
			if (substr ( $k, 0, 5 ) != "oauth")
				continue;
			if (is_array ( $v ))
				throw new ConsumerOAuthException ( 'Arrays not supported in headers' );
...
class OAuthServer { /*{{{*/
	...
	private function get_version(&$request) { /*{{{*/
		...
		if ($version && $version != $this->version) {
			throw new ConsumerOAuthException ( "OAuth version '$version' not supported" );
		}
		return $version;
	} /*}}}*/
	...
	private function get_signature_method(&$request) { /*{{{*/
		...
		if (! in_array ( $signature_method, array_keys ( $this->signature_methods ) )) {
			throw new ConsumerOAuthException ( "Signature method '$signature_method' not supported try one of the following: " . implode ( ", ", array_keys ( $this->signature_methods ) ) );
		}
	...
	private function get_consumer(&$request) { /*{{{*/
		...
		if (! $consumer_key) {
			throw new ConsumerOAuthException ( "Invalid consumer key" );
		}
		
		$consumer = $this->data_store->lookup_consumer ( $consumer_key );
		if (! $consumer) {
			throw new ConsumerOAuthException ( "Invalid consumer" );
		}
	...
	private function get_token(&$request, $consumer, $token_type = "access") { /*{{{*/
		...
		if (! $token) {
			throw new ConsumerOAuthException ( "Invalid $token_type token: $token_field" );
		}
		return $token;
	} /*}}}*/
	...
	private function check_signature(&$request, $consumer, $token) { /*{{{*/
		...
		if (! $valid_sig) {
			throw new ConsumerOAuthException ( "Invalid signature" );
		}
	} /*}}}*/
	...
	private function check_timestamp($timestamp) { /*{{{*/
		...
		if ($now - $timestamp > $this->timestamp_threshold) {
			throw new ConsumerOAuthException ( "Expired timestamp, yours $timestamp, ours $now" );
		}
	} /*}}}*/
	...
	private function check_nonce($consumer, $token, $nonce, $timestamp) { /*{{{*/
		...
		if ($found) {
			throw new ConsumerOAuthException ( "Nonce already used: $nonce" );
		}
	} /*}}}*/
...
class SimpleOAuthDataStore extends OAuthDataStore { /*{{{*/
	...
	function new_token($consumer, $type = "request") { /*{{{*/
		...
		if (! dba_insert ( "${type}_$key", serialize ( $token ), $this->dbh )) {
			throw new ConsumerOAuthException ( "doooom!" );
		}
		return $token;
	} /*}}}*/
